...
> **User evidence:** user research with Khrystelline (ex-Business ad major in finance student) and Chris Owyn (interviewed by Doniele Arys Antonio, team GC) confirms the acute job is **account reconciliation / balancing**, not transactions or reporting — and Khrystelline named the bottleneck ("Yung balancing"). For Chris Owyn side, Customer-service ideas are a future horizon (explicitly "malawak"); that broader surface is **out of MVP scope**. See `docs/user-research/users.md`.
